...
def parse_course_components(clean_text: str):
    ...
    header_match = re.search(r'^(.*?)\s+\((\d+(?:\.\d+)?)\s*cr\.\)', clean_text)
    if header_match:
        header = header_match.group(1).strip()
        credits = header_match.group(2)
        if ' ' in header:
    ...
    description_match = re.search(r'\)\s*(.*)', clean_text)
    description = description_match.group(1).strip() if description_match else clean_text
...
for page in range(1, 5):
    url = f"https://catalog.kirtland.edu/content.php?catoid=16&navoid=1635&filter[item_type]=3&filter[only_active]=1&filter[3]=1&filter[cpage]={page}"
    ...
    for link in course_links:
        ...
        ajax_url = (
            f"https://catalog.kirtland.edu/ajax/preview_course.php"
...
df.to_csv("data/kirtland_courses.csv", index=False)
