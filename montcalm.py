"""
Montcalm Community College Course Scraper
...
def parse_course_components(clean_text: str):
    ...
    match = re.search(
        r'^([A-Z]{2,6}\s*\d{3})\s*-\s*(.*?)\s+(\d+(?:\.\d+)?)\s+Credit:',
        clean_text
    )
    ...
    desc_match = re.search(r'Contact Hours:\s*.*?\]\s*(.*)', clean_text)
    if not desc_match:
        desc_match = re.search(r'Contact Hours:\s*(.*)', clean_text)

    if desc_match:
        description = desc_match.group(1).strip()
    else:
        description = re.split(r'Credit:', clean_text, maxsplit=1)[-1].strip()
...
for page in range(1, 5):
    url = f"https://catalog.montcalm.edu/content.php?catoid=34&navoid=3848&filter[item_type]=3&filter[only_active]=1&filter[3]=1&filter[cpage]={page}"
    ...
    for link in course_links:
        ...
        ajax_url = (
            f"https://catalog.montcalm.edu/ajax/preview_course.php"
...
df.to_csv("data/montcalm_courses.csv", index=False)
print("Done! Saved to data/montcalm_courses.csv")
